# tokens-cli-linux-x64-gnu: Packaging Note

> This Trellis package maps to `packages/cli-linux-x64-gnu/`, the npm shell
> that carries the prebuilt `tokens` binary for glibc Linux on x86_64
> (`tokens-cli-linux-x64-gnu` on npm). **There is no frontend here** — no
...
- In git: `packages/cli-linux-x64-gnu/package.json` only. The `bin/`
  directory is staged by CI at publish time and is never committed.
- Manifest contract: `os: ["linux"]`, `cpu: ["x64"]`, `libc: ["glibc"]`,
