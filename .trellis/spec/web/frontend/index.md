...
web unit tests; Playwright E2E is the T10 exception for browser journeys):
...
| `bun run test:teams` | T4 domain invariants (`scripts/check-teams-invariants.ts`); T7 FR-2 Teamboard pagination / multi-value `groupIds` live in the same runner |
| `bun run test:e2e` | Playwright LocaleToggle and Teamboard journeys (repo-root `tests/e2e/`, config `web/playwright.config.ts`) |
...
- `bun run lint` and `bun run typecheck` pass. Team/group domain and Teamboard loader changes also run `bun run test:teams`. LocaleToggle / Teamboard / user-visible nav journeys also run `bun run test:e2e`.
