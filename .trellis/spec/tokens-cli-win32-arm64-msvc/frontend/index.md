# tokens-cli-win32-arm64-msvc: Packaging Note

> This Trellis package maps to `packages/cli-win32-arm64-msvc/`, the npm shell
> that carries the prebuilt `tokens` binary for Windows on arm64 (MSVC
> toolchain; `tokens-cli-win32-arm64-msvc` on npm). **There is no frontend
...
- In git: `packages/cli-win32-arm64-msvc/package.json` only. The `bin/`
  directory is staged by CI at publish time and is never committed.
- Manifest contract: `os: ["win32"]`, `cpu: ["arm64"]`,
