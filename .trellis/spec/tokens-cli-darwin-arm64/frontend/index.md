# tokens-cli-darwin-arm64: Packaging Note

> This Trellis package maps to `packages/cli-darwin-arm64/`, the npm shell that
> carries the prebuilt `tokens` binary for macOS on Apple silicon
> (`tokens-cli-darwin-arm64` on npm). **There is no frontend here** — no JS/TS
...
- In git: `packages/cli-darwin-arm64/package.json` only. The `bin/` directory
  is staged by CI at publish time and is never committed.
- Manifest contract: `os: ["darwin"]`, `cpu: ["arm64"]`, `main: "bin/tokens"`,
