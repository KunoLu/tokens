# tokens-cli-linux-arm64-musl: Packaging Note

> This Trellis package maps to `packages/cli-linux-arm64-musl/`, the npm shell
> that carries the prebuilt `tokens` binary for musl Linux on arm64
> (`tokens-cli-linux-arm64-musl` on npm). **There is no frontend here** — no
...
- In git: `packages/cli-linux-arm64-musl/package.json` only. The `bin/`
  directory is staged by CI at publish time and is never committed.
- Manifest contract: `os: ["linux"]`, `cpu: ["arm64"]`, `libc: ["musl"]`,
