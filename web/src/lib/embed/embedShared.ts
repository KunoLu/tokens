...
export function todayMetricLine(options: {
  today: EmbedTodayUsage;
  ...
  const { today, palette, x, right, y, tokensCompact, costCompact } = options;
  return `<text x="${x}" y="${y}" fill="${palette.muted}" font-size="10" font-weight="600" font-family="${FIGTREE_FONT_STACK}">Today · UTC</text>
  ${fittedText({
    text: formatTodayInline(today, tokensCompact, costCompact),
    x: right,
    y,
    maxWidth: Math.max(120, right - x - 96),
    fill: palette.text,
    fontSize: 12,
    minFontSize: 8,
    fontWeight: 600,
...
export function applyEmbedColor(
  ...
  const brand = palette.scheme === "dark" ? grades[2] : p.grade2;
  return {
    ...palette,
    brand,
    glowColor: brand,
...
/**
 * The Tokens brand mark: a T whose stem is a column of rungs that shorten as
 * they rise. Kept in sync with `public/brand/tokens-mark.svg` by hand — an
 * embed is a standalone SVG served to third-party sites, so it cannot
 * reference the file and the paths have to be inlined.
 *
 * The source art is drawn in a 64 unit box but only inks the region below, so
 * scaling the box would leave the mark floating inside its own padding. The
 * transform maps the *ink* to `size` instead, which is what a caller placing
 * it next to 11px text actually means by "14 pixels tall".
 */
const MARK_INK = { x: 11.5, y: 12.5, width: 41, height: 39 };
...
export function brandIcon(
  ...
  // The old three-bar mark ran from `baselineY - 12` to `baselineY + 2`; the
  // two existing call sites are positioned against that, so keep the box.
  const translateX = x - MARK_INK.x * scale;
  const translateY = baselineY - 12 - MARK_INK.y * scale;

  return [
    `<g transform="translate(${translateX.toFixed(2)} ${translateY.toFixed(2)}) scale(${scale.toFixed(4)})" stroke="${color}" stroke-width="5" stroke-linecap="round" fill="none">`,
    `<path d="M14 15h36"/>`,
    `<path d="M32 15v34"/>`,
...
/** Rendered width of `brandIcon` at a given size, for laying out beside it. */
export function brandIconWidth(size = 14): number {
  return (MARK_INK.width / MARK_INK.height) * size;
}
...
export function cardHeader(options: CardHeaderOptions): string {
  ...
  // The lockup is mark + wordmark, flush to the right edge. The wordmark is
  // anchored to `right` so it lands exactly on the edge whatever it measures,
  // and the mark is placed by estimate ahead of it — if the estimate drifts the
  // gap between the two breathes slightly, which is invisible, whereas
  // anchoring the mark and letting the text run would overflow the card.
  const wordmarkWidth = estimateTextUnits(WORDMARK) * WORDMARK_SIZE;
  const markX = right - wordmarkWidth - MARK_GAP - brandIconWidth(MARK_SIZE);
  ...
  return [
    ...
    displayName
      ...
      : "",
    // Drawn in the theme accent rather than the muted grey of the wordmark it
    // leads: the mark is the one element on the card that should read as the
    // product, and #2F8FFF on the dark surface / #0969DA on the light one both
    // clear contrast against their own background, which one fixed colour
    // cannot do.
    //
    // brandIcon centres its ink on `baselineY - 5.5` at this size while an 11px
    // cap box centres on `y - 3.9`, so the baseline passed in is nudged down to
    // line the mark up with the wordmark rather than with nothing.
    brandIcon(markX, y + 1.6, palette.brand, MARK_SIZE),
    `<text x="${right}" y="${y}" fill="${palette.muted}" font-size="${WORDMARK_SIZE}" font-weight="600" text-anchor="end" font-family="${FIGTREE_FONT_STACK}">${WORDMARK}</text>`,
  ]
    .filter(Boolean)
    .join("\n  ");
}
