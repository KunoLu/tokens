...
function renderProfileCardSvg(
  ...
  const baseHeight = compact
    ...
    : graph
      ...
      : 186;
  const height = baseHeight + (options.today ? 26 : 0);
...
<svg data-template="classic" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Tokens profile stats for @${escapeXml(data.user.username)}">
  ${cardTextStyle()}
  ...
  ${options.today ? todayMetricLine({ today: options.today, palette, x, right, y: height - 34, tokensCompact: tokensFormat === "compact", costCompact: costFormat === "compact" }) : ""}
  ${cardFooter({
...
export function renderProfileEmbedErrorSvg(
  ...
  return `<?xml version="1.0" encoding="UTF-8"?>
<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Tokens embed error">
  ${cardTextStyle()}
  <g id="err-bg">
    ${cardSurface(width, height, palette)}
  </g>
  <text x="${x}" y="32" fill="${palette.text}" font-size="14" font-weight="600" font-family="${FIGTREE_FONT_STACK}">Tokens</text>
