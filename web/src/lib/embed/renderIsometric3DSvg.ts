...
<svg data-template="3d" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Tokens 3D contribution graph for ${safeUsername}">
  <title>${safeUsername} Tokens 3D contribution graph</title>
  <style>${faceCss}</style>
...
export function renderIsometric3DErrorSvg(
  ...
  return `<?xml version="1.0" encoding="UTF-8"?>
<svg data-template="3d-error" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Tokens 3D embed error">
